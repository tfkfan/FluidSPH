...
inline void SphFluidSolver::update_particle(Particle &particle) {
	if (length(particle.color_gradient) > 0.001f) {
		particle.force +=-material.sigma * particle.color_laplacian*normalize(particle.color_gradient);
	}
	Vector3f acceleration =   particle.force / particle.density - material.point_damping * particle.velocity / particle.mass;
